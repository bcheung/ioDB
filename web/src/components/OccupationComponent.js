...
const OccupationComponent = ({ data: { info, percentile, industries, states, metropolitan } }) => {
  ...
  const stateHeaders = states.headers.map((col) =>
    <th>{col}</th>
  );
  ...
  const metroHeaders = metropolitan.headers.map((col) =>
    <th>{col}</th>
  );
  ...
  return (
    <div style={styles.divStyle} className="OccupationComponent">
      <header style={styles.headerStyle} className="OccupationComponent-header">{info.occupation} {info.naics}</header>
